...
local function OnHack3Done(success)
    if success then 
        ...
        TriggerServerEvent('QBCore:Server:RemoveItem', Config.Hack3Item, 1, slot, info)
        ...
        if isActive then
            if isHacked3 then
                if not isLooted1 then 
                    ...
                    TriggerServerEvent('QBCore:Server:AddItem', Config.Loot1ItemA, amountA, slot, info)
                    TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[Config.Loot1ItemA], 'add', amountA)
                    TriggerServerEvent('QBCore:Server:AddItem', Config.Loot1ItemB, amountB, slot, info)
        ...
        if isActive then
            if isHacked3 then
                if not isLooted2 then 
                    ...
                    TriggerServerEvent('QBCore:Server:AddItem', Config.Loot2ItemA, amountA, slot, info)
                    TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[Config.Loot2ItemA], 'add', amountA)
                    TriggerServerEvent('QBCore:Server:AddItem', Config.Loot2ItemB, amountB, slot, info)
